...
**Answer：**\
`zcat`: It displays the contents of gzip-compressed files.\
`zless`: A file perusal filter for viewing compressed text files in a paginated form.\
`zmore`: It uses a different interface for file perusal.\
`zgrep`: It works on compressed files.
...
```{bash}
zcat < ~/desktop/mimic/hosp/admissions.csv.gz | tail -n +2 | awk -F',' '{print $6}' | sort | uniq | wc -l
...
```{bash}
zcat < ~/desktop/mimic/hosp/admissions.csv.gz | tail -n +2 | awk -F',' '{print $8}' | sort | uniq | wc -l
...
```{bash}
zcat < ~/desktop/mimic/hosp/admissions.csv.gz | tail -n +2 | awk -F',' '{print $10}' | sort | uniq | wc -l
...
```{bash}
zcat < ~/desktop/mimic/hosp/admissions.csv.gz | tail -n +2 | awk -F',' '{print $13}' | sort | uniq | wc -l
...
**Answer:**  
echo 'hello, world' \> test1.txt writes the string 'hello, world' to test1.txt. If test1.txt already exists, it will be overwritten.\
echo 'hello, world' \>\> test2.txt also writes the string 'hello, world' to test2.txt, but if test2.txt already exists, the string will be appended to the file instead of overwriting it. 
...
**Answer:**  
1.The output is the 5 lines from line 20 to line 24 of pg42671.txt.
...
For grading purpose, include a screenshot of Section 4.1.5 of the book here.
  **Answer:**
  ![image](/Users/luxier/Desktop/203b-hw/screenshot 2024-01-17 4.41.59.png)
