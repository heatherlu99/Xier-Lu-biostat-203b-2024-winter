...
setwd("/Users/luxier/Desktop/mimic/hosp/")
system.time({
  labevents_selected <- read_csv("labevents.csv.gz", col_select = c(subject_id, itemid, charttime, valuenum))
})
...
**Answer:** There are 24855910 lines in this new file.Not including header,There are 24855909 lines in this new file. The time is 3.732s.


```{bash}
#| eval: false
zcat < ~/desktop/mimic/hosp/labevents.csv.gz | awk -F, 'BEGIN {OFS=","} NR==1 || $5 == 50912 || $5 == 50971 || $5 == 50983 || $5 == 50902 || $5 == 50882 || $5 == 51221 || $5 == 51301 || $5 == 50931 {print $2, $5, $7, $10}' | gzip > ~/desktop/mimic/hosp/labevents_filtered.csv.gz
zcat <~/desktop/mimic/hosp/labevents_filtered.csv.gz | head -10
zcat < ~/desktop/mimic/hosp/labevents_filtered.csv.gz | wc -l
```

```{r}
system.time({
  labevents_filtered <- read_csv("/Users/luxier/Desktop/mimic/hosp/labevents_filtered.csv.gz")
})
```
...
**Answer:** The file size of "labevents.parquet" file is reported as 1.7G. The time is  0.935 s. The number of rows is 24855909. The first 10 rows are the same as in Q2.3.
...
arrow::write_dataset(labevents_frame, "/Users/luxier/desktop/mimic/hosp/labevents.parquet", format = "parquet")

system.time({
  labevents.parquet <- arrow::open_dataset("/Users/luxier/desktop/mimic/hosp/labevents.parquet") %>%
    dplyr::select(subject_id, itemid, charttime, valuenum) %>%
    dplyr::filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
    collect()
})
nrow(labevents.parquet)
print(labevents.parquet %>% head(10))

```

```{bash}
#| eval: true
ls -lh /Users/luxier/desktop/mimic/hosp/labevents.parquet
```
### Q2.6 DuckDB
...
labevents_parquet <- arrow::open_dataset("/Users/luxier/Desktop/mimic/hosp/labevents.parquet")
