...
labevents <- arrow::read_parquet("labevents_pq")
list.files("/Users/luxier/Desktop/203hw/hw3/labevents_pq")
...
# filter the data
patient_data <- list(patients, admissions, transfers, labevents, procedures_icd, diagnoses_icd) %>%
  lapply(function(df) df %>% filter(subject_id == 10013310))

patients <- patient_data[[1]]
admissions <- patient_data[[2]]
transfers <- patient_data[[3]]
labevents <- patient_data[[4]]
procedures_icd <- patient_data[[5]]
diagnoses_icd <- patient_data[[6]]

combined_data <- full_join(admissions, transfers, by = c("subject_id", "hadm_id")) %>%
  full_join(labevents, by = c("subject_id", "hadm_id")) %>%
  full_join(procedures_icd, by = c("subject_id", "hadm_id"))

ggplot(combined_data, aes(x = calendar_time, y = event_type)) + 
  geom_segment(aes(color = care_unit, size = is_icu_ccu), show.legend = FALSE) +  
  geom_point(data = combined_data[combined_data$event_type == "Lab", ], shape = 4) +  
  geom_point(data = combined_data[combined_data$event_type == "Procedure", ], aes(shape = procedure_type)) +  
  labs(
